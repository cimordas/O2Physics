...
struct flowJSPCAnalysisTask
{
	...
	Filter collFilter = (nabs(aod::collision::posZ) < eventCuts.cfgZvtxMax);
	// Filter trackFilter =  (aod::track::pt > trackCuts.cfgPtMin) && (aod::track::pt < trackCuts.cfgPtMax) && (nabs(aod::track::eta) < trackCuts.cfgEtaMax);
	...
	void process(soa::Filtered<MyCollisions>::iterator const& coll, MyTracks const& tracks) {
		// if (tracks.size() < eventCuts.cfgMultMin) return;
		// Int_t nTracks = tracks.size();

		float cent = -1.;
		switch (eventCuts.cfgCentEst) {
		...
		Int_t cBin = spcAnalysis.GetCentBin(cent);
		printf("%d\n", cBin);
